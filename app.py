...
def window_bounds(df, default_years_back=3):
    """Start/end dates of the default zoomed window, for computing a y-range that matches what's
    actually visible on open (Plotly's y-autorange otherwise fits the FULL series -- including the
    2020 pandemic spike -- even when the x-axis opens zoomed to a recent window)."""
...
st.caption("Q3 FY2026 – Q2 FY2027 point estimates, built from public data only. See the Overview tab for methodology.")

tab_overview, tab_trend, tab_industry, tab_macro, tab_explorer = st.tabs(
    ["Overview & Methodology", "Comp Sales Trend", "Industry Trends", "Macro Trends", "Data Explorer"]
)
...
# =====================================================================
with tab_trend:
    st.subheader("Comp Sales — Actual vs. Projected")
    st.caption("Solid = actual reported comp sales. Dashed = regression point estimate. Use the range slider or preset buttons below the chart to zoom past the 2020 pandemic spike.")
    ...
    for company, color in COMPANY_COLOR.items():
        ...
        fig.add_trace(go.Scatter(
            ...
            name=f"{company} (actual)", mode="lines", line=dict(color=color, width=2.2),
            customdata=actual["Fiscal Quarter"],
            hovertemplate="%{customdata}: %{y:.1f}%<extra>" + company + " actual</extra>",
        ))
        fig.add_trace(go.Scatter(
    ...
    st.plotly_chart(fig, width='stretch')
    st.caption("Legend: solid line = actual; dashed line = projected (regression point estimate). Drag the range slider or use the 1Y/3Y/5Y/All buttons to change the visible window.")
...
with tab_industry:
    ...
    for col, company in ((col_hd, "HD"), (col_low, "LOW")):
        with col:
            ...
            fig2 = go.Figure()
            fig2.add_trace(go.Scatter(x=actual_l["Quarter Start"], y=actual_l[comp_col] * 100, name="Comp Sales % (actual)",
                                       mode="lines", line=dict(color=BLUE if company == "HD" else ORANGE, width=2),
                                       customdata=actual_l["Fiscal Quarter"], hovertemplate="%{customdata}: %{y:.1f}%<extra>Comp (actual)</extra>"))
            ...
            fig2.add_trace(go.Scatter(x=l["Quarter Start"], y=l["LIRA Rate of Change"] * 100, name="LIRA Rate of Change",
            ...
            fig2.update_layout(**base_layout(
                ...
                yaxis2=dict(title="LIRA Rate of Change %", ticksuffix="%", overlaying="y", side="right", showgrid=False,
            ...
            st.caption("LIRA's 4 rightmost quarters are Harvard's own projections, shown alongside our regression estimate for the same period.")
...
with tab_macro:
    ...
    st.caption("HD (left) and LOW (right). Correlation heatmap shows which macro predictor is strongest at which forecast horizon — the basis for predictor selection in the point estimates.")
    ...
    for col, company in ((col_hd, "HD"), (col_low, "LOW")):
        with col:
            ...
            st.markdown("**Comp Sales vs. Mortgage Rate**")
            fig1 = go.Figure()
            ...
            fig1.add_trace(go.Scatter(x=d_actual["Quarter Start"], y=d_actual["Mortgage Rate (%)"], name="Mortgage Rate",
                                       mode="lines", line=dict(color=YELLOW, width=2), yaxis="y2",
            ...
            fig1.update_layout(**base_layout(
                height=340, yaxis=dict(title="Comp Sales %", ticksuffix="%", gridcolor=GRID,
                                        range=windowed_range(d, "Comp Sales % (US)", _start, _end, scale=100)),
                yaxis2=dict(title="Mortgage Rate %", ticksuffix="%", overlaying="y", side="right", showgrid=False,
                            range=windowed_range(d_actual, "Mortgage Rate (%)", _start, _end, scale=1)),
                xaxis=dated_xaxis(default_years_back=3, df=d),
            ))
            st.plotly_chart(fig1, width='stretch', key=f"mort_{company}")

            st.markdown("**Mortgage Rate vs. Home Sales**")
            fig2 = go.Figure()
            ...
            fig2.add_trace(go.Scatter(x=d_actual["Quarter Start"], y=d_actual["Mortgage Rate (%)"], name="Mortgage Rate",
                                       mode="lines", line=dict(color=RED_NEG, width=2), yaxis="y2",
                                       customdata=d_actual["Fiscal Quarter"], hovertemplate="%{customdata}: %{y:.2f}%<extra>Mortgage</extra>"))
            _start, _end = window_bounds(d_actual, 3)
            fig2.update_layout(**base_layout(
                height=340, barmode="stack",
                yaxis=dict(title="Home Sales (M, SAAR)", gridcolor=GRID,
...
with tab_explorer:
    ...
    st.caption("Browse the full Quarterly Data tab. Filter and download as needed.")
